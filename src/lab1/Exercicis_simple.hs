...
sumaMenores :: Int -> [Int] -> Int
sumaMenores n [] = 0
sumaMenores n (front : rest) = if (front < n ) then (front + (sumaMenores n rest)) else (sumaMenores n rest)

listaNumeros :: [Int]
listaNumeros = [1,2,5,9,8,4,5,3,6,7,2,3,5]
...
maximo :: [Int] -> Int                          -- RECURSIVA ACUMULATIVA EL DEFAULT HA DE SER MIN INT.
...
lista_enteros :: Int -> [Int] -> [Int]
lista_enteros _ [] = []
lista_enteros n (x:xs) = if (x>0 && x<n) then
        (x: (lista_enteros n xs)) else
        (lista_enteros n xs)
 
------------------------------------------------

-- Crear una funcion listar_entre(inicio,fin,listaX) que devuelva 
        -- una lista formada por los enteros entre inicio y fin de la lista listaX.

listar_entre :: Int -> Int -> [Int] -> [Int]
listar_entre _ _ [] = []
listar_entre ini fin (front:rest) = if (front>ini && front<fin) then
        (front : (listar_entre ini fin rest)) else
        (listar_entre ini fin rest) 
------------------------------------------------
...
listar_entre2 _ _ [] = []
listar_entre2 ini fin (front:rest) = if (front>ini && front<fin && (front `mod` 2 == 0)) then 
        (front : (listar_entre2 ini fin rest)) else 
        (listar_entre2 ini fin rest) 
...
-- Implementa la funcion "suma_div" donde a partir de un numero se obtenga
        -- la suma de todos sus divisores sin incluir a si mismo. Hacedlo con recursividad de pila y acumulativa
...
-- recursividad acumulativa -> es disposa d'un paramtre adcional acumulador que en aquest cas s'anomena suma
...
-- Implementar la función "potencia" que recibe por parámetro un numero entero y el numero al que lo queremos elevar
        -- y nos devuelve el resultado de aplicar la operacion. 
...
-- Implementar la función "central" que recibe por parámetro una lista y devuelve una estructura 
        -- con tres elementos: 
